...
# test vars
PD_PATH = './application/.testdata/data'
OUL_PATH = './'
...
class Machine:
    ...
    def get_process_data(machine, process_id, t_start, t_end):
        ...
        # converts the 'cognex_timestamp' column of data
...
class iTrakPoucher:
    ...
    def slice_shifts(self):
        ...
        self.first_shift = DataBlock(
            self.MACHINE_ID, self.PROCESS_ID, self.startup, self.shift_change)
        self.second_shift = DataBlock(
            self.MACHINE_ID, self.PROCESS_ID, self.shift_change, self.shutdown)
    ...
    def calculate_stats(self):
        ...
        # empty flights
        db.empties = db.data[db.data['part_present'] == 0]
        db.stats['empty'] = db.empties.__len__()
        db.stats['empty_perc'] = (db.stats['empty'] / db.data.__len__())

        # parts
        db.parts = db.data[db.data['part_present'] != 0]
        db.stats['parts'] = db.parts.__len__()

        # reworks
        db.reworks = db.parts[db.parts.duplicated('serial_number')]
    ...
    def calculate_stop_stats(self):
        ...
        db.stats['avg_rate_upm'] = db.parts.__len__() / \
            (db.total_run_time / 60)
    ...
    def calculate_yields_1s(self):
        ...
        db.yields['upm_rolling3s'] = db.yields['upm'].rolling(3).mean()
        db.yields['upm_rolling5s'] = db.yields['upm'].rolling(5).mean()
        db.yields['upm_rolling15s'] = db.yields['upm'].rolling(15).mean()
    ...
    def draw_yield_plot(
        ...
        plt.fill_between(
            yields.index, 0, yields['count'], lw=0.01, color='xkcd:cerulean')
    ...
    def draw_rate_plot(
        ...
        plt.axvspan(
            t_start, data[data['part_present'] == 1].index[0],
            color='lightgrey', lw=0.01, alpha=1.0
        )
        plt.axvspan(
            data[data['part_present'] == 1].index[-1], t_end,
            color='lightgrey', lw=0.01, alpha=1.0
        )
...
class DataBlock:
    ...
    def __init__(self, machine_id, process_id, t_start, t_end, shift_change=None):
...
for date in dates:
    ...
    startup = datetime.datetime.combine(
        date, datetime.time(6, 0, 0, 0)
    )
    shutdown = datetime.datetime.combine(
        date, datetime.time(23, 0, 0, 0)
    )
